    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 ">Kelola jadwal kegiatan kelurahan</p>
        </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <p className="p-6 text-center text-slate-500 ">Belum ada agenda.</p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100 ">
                {list.map((item) => (
                  <tr key={item.id}>
                    <td className="px-4 py-3 font-medium text-slate-900  max-w-xs truncate">{item.judul}</td>
                    <td className="px-4 py-3 text-slate-500 ">{formatDate(item.tanggal)}</td>
                    <td className="px-4 py-3">
                      <span className={`badge ${item.status === 'upcoming' ? 'bg-primary-100 text-primary-700  ' : 'bg-slate-100 text-slate-600  '}`}>
                        {item.status === 'upcoming' ? 'Akan Datang' : 'Selesai'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => openEdit(item)} className="w-8 h-8 rounded-lg bg-slate-100  flex items-center justify-center text-slate-600  hover:bg-primary-100 hover:text-primary-700   transition-colors"><Pencil className="w-4 h-4" /></button>
                        <button onClick={() => setDeleteId(item.id)} className="w-8 h-8 rounded-lg bg-slate-100  flex items-center justify-center text-slate-600  hover:bg-red-100 hover:text-red-700   transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}>
        <div className="p-6 md:p-8">
          <h2 className="text-xl font-bold text-slate-900  mb-5">{editingId ? 'Edit Agenda' : 'Tambah Agenda'}</h2>
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700  mb-1.5">Waktu (teks)</label>
      ...
      <Modal isOpen={deleteId !== null} onClose={() => setDeleteId(null)}>
        <div className="p-6 md:p-8 text-center">
          <h2 className="text-lg font-bold text-slate-900  mb-2">Hapus agenda ini?</h2>
