...
const statusConfig = {
  ...
  dibaca: { label: 'Dibaca', className: 'bg-yellow-100 text-yellow-700  ' },
  ditindaklanjuti: { label: 'Ditindaklanjuti', className: 'bg-green-100 text-green-700  ' },
};
...
function PesanKontakAdmin() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900  mb-1">Pesan Kontak</h1>
      <p className="text-slate-500  mb-6">Pesan masuk dari formulir kontak website</p>

      <div className="card overflow-hidden">
        {loading ? (
          <p className="p-6 text-center text-slate-500 ">Memuat pesan...</p>
        ) : list.length === 0 ? (
          <p className="p-6 text-center text-slate-500 ">Belum ada pesan masuk.</p>
        ) : (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className={`font-semibold text-sm ${item.status === 'baru' ? 'text-slate-900 ' : 'text-slate-600 '}`}>
                      {item.nama}
                    </span>
                  ...
                  <p className="text-sm text-slate-600  truncate">{item.subjek}</p>
                </div>
      ...
      <Modal isOpen={selected !== null} onClose={() => setSelected(null)}>
        {selected && (
          <div className="p-6 md:p-8">
            ...
            <h2 className="text-lg font-bold text-slate-900  mb-1">{selected.subjek}</h2>
            <p className="text-sm text-slate-500  mb-4">
              Dari: <span className="font-medium">{selected.nama}</span> ({selected.email})
            </p>

            <div className="p-4 rounded-xl bg-slate-50  text-sm text-slate-700  leading-relaxed whitespace-pre-wrap mb-6">
              {selected.pesan}
            </div>
