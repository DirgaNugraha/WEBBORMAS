  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 ">Potensi Kelurahan</h1>
          <p className="text-slate-500 ">Kelola potensi pertanian, perikanan, UMKM, wisata, dll</p>
        </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {loading ? (
          ...
          <p className="text-slate-500  col-span-full text-center py-6">Belum ada data potensi.</p>
        ) : (
          list.map((item) => {
              ...
              <div key={item.id} className="card overflow-hidden">
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="badge bg-secondary-100 text-secondary-700  ">{item.kategori}</span>
                  </div>
                  <p className="font-semibold text-sm text-slate-900  mb-2">{item.nama}</p>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}>
        <div className="p-6 md:p-8">
          <h2 className="text-xl font-bold text-slate-900  mb-5">{editingId ? 'Edit Potensi' : 'Tambah Potensi'}</h2>
      ...
      <Modal isOpen={deleteId !== null} onClose={() => setDeleteId(null)}>
        <div className="p-6 md:p-8 text-center">
          <h2 className="text-lg font-bold text-slate-900  mb-2">Hapus potensi ini?</h2>
