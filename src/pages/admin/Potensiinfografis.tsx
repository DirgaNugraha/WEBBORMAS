...
const emptyForm = { label: '', value: '', icon: 'Wheat', color: 'primary', urutan: 0 as number | '' };

function PotensiInfografisAdmin() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    const data = await adminService.list<StatItem & { id: string }>('potensi_infografis', 'urutan', true);
    setList(data);
    setLoading(false);
  }, []);
  ...
  const handleSave = async () => {
    if (!form.label.trim() || !form.value.trim()) { setFormError('Label dan nilai wajib diisi.'); return; }
    setSaving(true);
    setFormError('');
    ...
    const { error } = editingId
      ? await adminService.update('potensi_infografis', editingId, payload)
      : await adminService.create('potensi_infografis', payload);
    setSaving(false);
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    await adminService.remove('potensi_infografis', deleteId);
    setDeleteId(null);
    loadData();
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Infografis Potensi</h1>
          <p className="text-slate-500 dark:text-slate-400">6 kartu infografis di halaman potensi kelurahan</p>
        </div>
        <button onClick={openCreate} className="btn-primary shrink-0"><Plus className="w-4 h-4" /> Tambah Infografis</button>
      </div>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)}>
        <div className="p-6 md:p-8">
          <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-5">{editingId ? 'Edit Infografis' : 'Tambah Infografis'}</h2>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Label</label>
              <input type="text" value={form.label} onChange={(e) => setForm({ ...form, label: e.target.value })} placeholder="contoh: Lahan Pertanian" className="input-field" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Nilai</label>
              <input type="text" value={form.value} onChange={(e) => setForm({ ...form, value: e.target.value })} placeholder="contoh: 450 ha" className="input-field" />
            </div>
      ...
      <Modal isOpen={deleteId !== null} onClose={() => setDeleteId(null)}>
        <div className="p-6 md:p-8 text-center">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-2">Hapus infografis ini?</h2>
...
export default PotensiInfografisAdmin;